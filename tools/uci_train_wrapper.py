"""Run UCI training benchmarks through DeepFlow/STG/MLSynth wrappers and plot results."""
...
WRAPPER_OUT_BASE = TOOLS_ROOT / "comp" / "wrapper_outputs" / "uci_train"
...
from validation_scripts import uci_train  # noqa: E402
...
def _extract_actual_time(spec: vh.ValidationSpec, actual_lookup: Dict[Tuple[str, int, int, int, int], float]) -> Optional[float]:
    meta = spec.metadata or {}
    variant_val = str(meta.get("variant") or "")
    key_lower = (
        variant_val.lower(),
        int(meta.get("tp", 0)),
        int(meta.get("pp", 0)),
    ...
    return actual_lookup.get(key_lower) if key_lower in actual_lookup else actual_lookup.get(key_raw)


def _short_label(spec: vh.ValidationSpec) -> str:
    meta = spec.metadata or {}
    variant = str(meta.get("variant") or "").upper()
    tp = meta.get("tp", "?")
    pp = meta.get("pp", "?")
    cp = meta.get("cp", "?")
    dp = meta.get("dp", "?")
    return f"{variant} TP={tp} PP={pp} CP={cp} DP={dp}"
...
def run_spec(
    ...
    with _with_output_root(spec_root):
        for name in tools:
            print(f"[uci_train_wrapper] Running {name} for {label}")
...
def _load_existing_runs(output_root: Path, include_variants: Optional[Sequence[str]] = None) -> List[SpecRun]:
    runs: List[SpecRun] = []
    include_set = {v.lower() for v in include_variants} if include_variants else set()
    ...
    for path in result_files:
        try:
            ...
            variant = str(meta.get("variant") or "").lower()
...
def plot_results(spec_runs: List[SpecRun], tools: Sequence[ToolName], path: Path) -> Path:
    ...
    ax.set_title("UCI training comparison (DeepFlow/STG/MLSynth vs. actual)")
    ax.legend()
...
DEFAULT_VARIANTS: Tuple[str, ...] = ("ddp",)


def _filter_cp_specs(specs: Sequence[vh.ValidationSpec]) -> List[vh.ValidationSpec]:
    filtered: List[vh.ValidationSpec] = []
    for spec in specs:
        meta = spec.metadata or {}
        cp_val = meta.get("cp", 0)
        try:
            cp_int = int(cp_val)
...
def _filter_tp_specs(specs: Sequence[vh.ValidationSpec]) -> List[vh.ValidationSpec]:
    filtered: List[vh.ValidationSpec] = []
    for spec in specs:
        meta = spec.metadata or {}
        tp_val = meta.get("tp", 0)
        try:
            tp_int = int(tp_val)
        except (TypeError, ValueError):
            tp_int = 0
...
def run(
    *,
    ...
    hw_cfg = str(uci_train.DEFAULT_HW_CONFIG)
    model_cfg = str(uci_train.DEFAULT_MODEL_CONFIG)

    output_slug = f"{_slugify(Path(model_cfg).stem)}__{_slugify(Path(hw_cfg).stem)}"
    output_root = WRAPPER_OUT_BASE / output_slug
    output_root.mkdir(parents=True, exist_ok=True)

    spec_runs: List[SpecRun] = []
    if collect_only or no_gen:
        spec_runs = _load_existing_runs(output_root, include_variants=variants)
        spec_runs = [
            run
            for run in spec_runs
            if (run.spec.metadata or {}).get("cp", 0) in {0, 1, "0", "1"}
            and str((run.spec.metadata or {}).get("tp", 0)) not in {"4", 4}
        ]
        actual_lookup: Dict[Tuple[str, int, int, int, int], float] = {}
        if not spec_runs and no_gen:
            raise ValueError("no_gen requested but no existing results were found to reuse.")
        if no_gen:
            print("[uci_train_wrapper] no_gen set: reusing existing results; no ET generation will be performed.")
    else:
        specs, actual_lookup = uci_train.build_specs(variants, model_cfg, hw_cfg)
        specs = _filter_tp_specs(_filter_cp_specs(specs))
        if not specs:
    ...
    if emit_logs:
        print("[uci_train_wrapper] Completed runs:")
        ...
        if plot_path:
            print(f"[uci_train_wrapper] Plot saved to {plot_path}")
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(description="Run or collect UCI training comparisons.")
    ...
    parser.add_argument("--num_configs", type=int, default=None, help="Limit number of configs (1=first only, etc.).")
    ...
    if result.get("spec_runs"):
        combined_plot = plot_results(result["spec_runs"], ["deepflow", "stg", "mlsynth"], WRAPPER_OUT_BASE / "train_combined.png")
        print(f"[uci_train_wrapper] Combined plot saved to {combined_plot}")
