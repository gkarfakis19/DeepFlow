            ...
            for attr in node.attr:
                if attr.name == "outputs":
                    ...
                    if which in ('str_list', 'string_list'):
                        if which == 'str_list':
                            values = list(attr.str_list.values)
                        else:
                            values = list(attr.string_list.values)
                    ...
                    if which in ('str_list', 'string_list'):
                        ...
                        for i in range(0, len(values), 2):
                            tensor_name = values[i]
                            parsed.consumers.setdefault(tensor_name, set()).add(int(node.id))
    finally:
        fh.close()
    return parsed
...
def _graph_for_et(parsed: ParsedET, name: str) -> Digraph:
    ...
    for node_id, node in parsed.nodes.items():
        ...
        dot.node(str(node_id), '\n'.join(label_lines), style="filled", fillcolor=color, shape="box", fontsize="9")
